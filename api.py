...
async def handle_upload_photo(request):
    ...
    Sends to Telegram and returns file_id.
    """
    if not _is_authorized(request):
        return _unauthorized(request)
    ...
    try:
        ...
        # Wrap bytes in BytesIO so PTB treats it as a file upload
        ...
        return web.json_response({
            'data': {
                'id': file_id,
                'url': f"tg:{file_id}",  # Internal protocol for our bot
                'direct_url': f"tg:{file_id}", # Legacy compat
                'display_url': f"tg:{file_id}" # Legacy compat
            },
            'success': True,
            'status': 200
        }, headers=_get_cors_headers(request))
