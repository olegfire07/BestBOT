    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
            for idx, item in enumerate(items, 1):
                ...
                logger.info(f"Item {idx}/{len(items)}: photo_url={photo_url[:50] if photo_url else 'None'}...")
                
                if photo_url:
                    # Use PhotoService to download (supports tg: and http)
                    file_path = await PhotoService.download_photo(photo_url, context.bot, client)
                    ...
                    if file_path:
                        ...
                        logger.info(f"Added photo entry: {photo_entry}")
                    else:
                        logger.error(f"Failed to download photo for item {idx}: {photo_url}")
                else:
